%% Convert the Pulseq-generated MPRAGE Siemens .dat data to ISMRMRD data
...
%% load .seq file and read label and sequence definitions
clear; close all ; clc ;
seq = mr.Sequence() ;                       % Create a new sequence object
seqfilename = 'mprage_challenge.seq' ;
...
datafile = dir('meas_MID00203_FID24417_pulseq_we_mprage_sag_p2_1mm_iso.dat') ;
...
% calculate other sequence parameters based on Nsample
...
%%
% Output file Name
filename = 'pulseq_mprage_data.h5';
...
for i = 1:Nscan
    ...
    if REFlbl(i) == 1
        if IMAlbl(i) == 1
            ...
            % in ACS block but not part of the regular undersampling pattern
            acqblock.head.flagSet('ACQ_IS_PARALLEL_CALIBRATION', i) ;
        end
    end
    % fill the data
    acqblock.data{i} = squeeze(totalData(:,1:Ncoil,i)) ;
end
...
header.measurementInformation.measurementID = '123_456_789_10' ;
header.measurementInformation.patientPosition = 'xxx' ;
