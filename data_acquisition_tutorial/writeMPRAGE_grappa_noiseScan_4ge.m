...
rf_fs_peak = max(abs(rf180.signal))/gamma_H1 ; % [uT]
rf_peak = max(abs(rf.signal))/gamma_H1 ; % [uT]
disp(['The peak rf180 amplitude = ', num2str(rf_fs_peak), ' uT']) ;
...
adcDwell=floor(ro_dur/(N(ax.n1)*ro_os)*1e7)*1e-7;
...
seq.write('mprage_noiseScan.seq') ;      % Write to pulseq file
%seq.install('siemens');
