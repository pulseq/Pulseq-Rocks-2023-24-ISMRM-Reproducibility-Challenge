...
%             gy = mr.makeTrapezoid('y', 'Area', phaseAreas(i), 'Duration', grPredur,'system',system) ;
%         else
%             gy = mr.makeTrapezoid('y', 'Area', 0, 'Duration', grPredur,'system',system) ;
%         end
        % loop over slices
...
%                 seq.addBlock(gr) ;
%             end
%             seq.addBlock(gx_spoil, gy, gz_spoil) ;
            if (i>0) % semi-negative index -- dummy scans
                seq.addBlock(adc, gr) ;
                seq.addBlock(mr.makeLabel('INC', 'SLC', 1)) ;
...
seq.write('QA_loc.seq')       % Write to pulseq file
