% this is an experimental sequence which allows for TE and TR settings using the
% 'softdelay' estension. It is based on the high-performance EPI sequence
...
Nrep = 1 ;
TR = 3000e-3 ;
max_TE = 100e-3;           % needed for soft delays to resolve TE/TR dependency
...
for r=1:Nrep
    ...
    for s=1:Nslices
        ...
        if Nnav>0
            ...
            % SD blocks must have a minimal non-zero duration at the design
            % state, which may later be eliminated by setting the soft
            % delay parameter value TE exactly to min_TE
            seq.addBlock(10e-6,mr.makeSoftDelay(0,'TE','offset',-min_TE)); 
            seq.addBlock(gyPre,  ...
       ...
       % softdelay 2nd TE-related block that is needed for resolving the
       % TE/TR dependency. The goal is to keep the duration of the 'core'
       % of the TR loop independent of the TE setting. It is quite a tricky
       % thing to calculate... but checkTiming() helps to find out if it was right   
       seq.addBlock(max_TE-min_TE-10e-6,mr.makeSoftDelay(0,'TE','factor',-1,'offset',max_TE));
       if s==1 && r==1
           durPerSlc=sum(seq.blockDurations); % note the actual minimal TR-per-slice duration
           assert(abs(minTR_1slice+max_TE-min_TE-durPerSlc)<1e-9);
       end
       % softdelay for setting the TR
       seq.addBlock(TRdelay_perSlice,mr.makeSoftDelay(1,'TR','offset',-durPerSlc,'factor',Nslices));
    end
...
seq.plot('Label', 'SEG,LIN,SLC', 'timeRange', [0 TR]) ;             % Plot all sequence waveforms
