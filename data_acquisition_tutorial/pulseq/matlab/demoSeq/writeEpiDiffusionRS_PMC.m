...
lblPmcOn=mr.makeLabel('SET','PMC',1);
lblPmcOff=mr.makeLabel('SET','PMC',0);
...
for s=1:Nslices
    ...
    seq.addBlock(rf,gz,trig,lblPmcOn);
    seq.addBlock(mr.makeDelay(delayTE1),gDiff,lblPmcOff);
    rf180.freqOffset=gz180.amplitude*thickness*(s-1-(Nslices-1)/2);
...
seq.plot('showBlocks',1,'stacked',1);             % Plot sequence waveforms
...
%% evaluate label settings (e.g. to visualize the PMC flag)
lbls=seq.evalLabels('evolution','blocks');
...
legend(lbl_names(:));
title('evolution of the PMC flag');
xlabel('block number');

%% now with a valid time axis and showing block timing
...
%% another pretty plot option e.g. for publiocations
